...
* When an organization is deleted, **all associated data is permanently removed**: agents, tasks, and runs. This is a hard, irreversible cascade delete.

### Cost & Token Aggregation
Organizations do not store cost or token counters. All aggregate figures (`consumed_input_tokens`, `consumed_output_tokens`, `total_cost`) are **computed at query time** by summing across all Runs belonging to the organization.
...
* **Organization cost** = SUM of all Runs where `organization_name` matches.
* **System cost** = SUM of all Runs across all organizations.

### Immutability
Task records are never deleted individually. Soft-deleted agents preserve their Run history for accurate cost attribution. If an entire organization is deleted, all associated Runs, Tasks, and Agent records are permanently removed.
